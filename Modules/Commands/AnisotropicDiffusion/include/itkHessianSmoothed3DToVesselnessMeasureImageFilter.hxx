...
//         vnl_math_abs( Lambda2) < EPSILON  || 
 //        vnl_math_abs( Lambda3 ) < EPSILON )
    ...
    double vesMeasure_1  = ( 1 - vcl_exp( -1.0 * (( vnl_math_abs( S )) / ( ( m_Gamma)))));
    
    double vesMeasure_2 = vcl_exp(-1.0*(( vnl_math_abs(A) ) / (  ( m_Alpha))));
    
    double vesMeasure_3  = 
         vcl_exp ( -1.0 * ((vnl_math_abs( B )) /  (  (m_Beta))));
         #if 0
    double vesMeasure_4 = vcl_exp ( -1.0 * ( 4* vnl_math_abs( m_C )) / 
                                   Lambda3Sqr);
    ...
    double vesMeasure_1  = ( 1 - vcl_exp( -1.0 * (( vnl_math_sqr( S )) / ( 2.0 * ( GammaSqr)))));
    
    double vesMeasure_2 = vcl_exp(-1.0*(( vnl_math_sqr(A) ) / ( 2.0 * ( AlphaSqr))));
    
    double vesMeasure_3  = 
         vcl_exp ( -1.0 * ((vnl_math_sqr( B )) /  ( 2.0 * (BetaSqr))));
         
    double vesMeasure_4 = vcl_exp( -1.0 * ( 2.0 * vnl_math_sqr( m_C )) / Lambda3Sqr);
    //double vesMeasure_4=1;
    double vesselnessMeasure = 
         vesMeasure_1 * vesMeasure_2 * vesMeasure_3 * vesMeasure_4; 
