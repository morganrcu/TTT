...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::ComputSpeedImage() {
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::InitDefGraph() {
	m_Descriptor = ttt::TissueDescriptor<dim>::New();

	//m_Descriptor->m_SkeletonGraph = boost::shared_ptr<ttt::SkeletonGraph<dim>>(new ttt::SkeletonGraph<dim>);


	for (typename ttt::AdherensJunctionVertices<dim>::iterator it =
			m_Locations->begin(); it != m_Locations->end(); it++) {
		itk::Point<float, dim> pos;

		m_Plateness->TransformIndexToPhysicalPoint((*it)->GetPosition(), pos);

		//ttt::SkeletonPoint<dim> pt = ttt::SkeletonPoint<dim>(pos);
		m_Descriptor->GetAJGraph().AddVertex(pos);
		//SkeletonVertexType n = boost::add_vertex(static_cast<ttt::SkeletonPoint<dim>>(pt),m_Descriptor->GetAJGraph());
...
template<> void ttt::AdherensJunctionSegmentationDijkstraCommand<3>::GetNeighbors(const itk::Index<3> & index, std::vector<itk::Index<3> > & neighbors) {
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::DoFastMarching() {

	typedef itk::Image<int,dim> VoronoiImageType;
	...
	for (typename  ttt::AdherensJunctionVertices<dim>::iterator it = m_Locations->begin();
			it != m_Locations->end(); it++) {
		typename PlatenessImageType::IndexType seedPosition = (*it)->GetPosition();
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::PruneLevelSet(
		double value) {
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::BuildGraph() {
#if 0
			...
			for (typename std::vector<Index>::iterator itNeigh = neighbors.begin();
				...
				if (neighValue != itk::NumericTraits<unsigned long>::max()
					...
					if (!boost::edge(currentValue, neighValue,
							m_Descriptor->GetAJGraph()).second
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::StoreLabels() {
#if 0
...
template<int dim> void ttt::AdherensJunctionSegmentationDijkstraCommand<dim>::Do() {
