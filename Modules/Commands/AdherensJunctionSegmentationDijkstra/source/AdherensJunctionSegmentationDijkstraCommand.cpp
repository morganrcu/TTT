...
void ttt::AdherensJunctionSegmentationDijkstraCommand::InitDefGraph() {
	...
	m_Descriptor->m_SkeletonGraph = boost::shared_ptr<ttt::SkeletonGraph>(
			new ttt::SkeletonGraph);

	for (typename ttt::AdherensJunctionVertices::iterator it = m_Locations->begin();
...
void ttt::AdherensJunctionSegmentationDijkstraCommand::GetNeighbors(const itk::Index<3> & index,std::vector<itk::Index<3> > & neighbors){
...
void ttt::AdherensJunctionSegmentationDijkstraCommand::DoFastMarching(){
	...
	int k=0;
	for(ttt::AdherensJunctionVertices::iterator it=m_Locations->begin();it!=m_Locations->end();it++){
...
double ttt::AdherensJunctionSegmentationDijkstraCommand::ComputePath(const SkeletonVertexType & a, const SkeletonVertexType & b){
	...
	while (!trialHeap.empty() && !found) {
		...
		if(currentIndex==targetIndex){
			...
			for(std::vector<Index>::iterator it = neighs.begin();it!=neighs.end();it++){
				Index neigh = *it;
				char neighcolor = colors->GetPixel(neigh);
				if (neighcolor == COLOR_BLACK) continue;
				if(m_Labels->GetPixel(neigh)==a || m_Labels->GetPixel(neigh)==b){
					...
					if (candDist < neighDist) {
						...
						if (neighcolor == COLOR_GRAY) {
							typename HeapType::iterator findres = std::find(trialHeap.begin(), trialHeap.end(),current);
							findres->SetValue(candDist);
							std::update_heap_pos(trialHeap.begin(),trialHeap.end(), findres);
						} else {
