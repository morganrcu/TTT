...
void ttt::JSONTissueTrackingProject2::StoreFrameInfo(unsigned int frame){
	...
	Json::Value root;
	Json::StyledWriter writer;
	std::cout << "JSON StepsPlateness" << this->m_PlatenessSteps[frame] << std::endl;
	...
	std::stringstream fileNameStream;
	fileNameStream << m_ProjectDirectory << "/" << "frame_T"<< frame << ".json";
...
void ttt::JSONTissueTrackingProject2::LoadFrameInfo(unsigned int frame){
	...
	std::cout << "PlatenessSteps: " << this->m_PlatenessSteps[frame] << std::endl;
...
typename AdherensJunctionVertices::Pointer ttt::JSONTissueTrackingProject2::GetAdherensJunctionVertices(unsigned int frame){
	...
	std::stringstream fileNameStream;
	fileNameStream << m_ProjectDirectory << "/" << "vertices_T" << frame << ".json";
	...
	typename AdherensJunctionVertices::Pointer vertices = AdherensJunctionVertices::New();

	int numvertices=root["Vertices"].size();

	for(int i=0;i<numvertices;i++){
		ttt::AdherensJunctionVertex::Pointer newVertex =ttt::AdherensJunctionVertex::New();
...
bool ttt::JSONTissueTrackingProject2::IsAdherensJunctionVerticesAvailable(unsigned int frame){
 ...
 typename  ttt::TissueDescriptor::Pointer ttt::JSONTissueTrackingProject2::GetTissueDescriptor(unsigned int frame){
	 ...
	 for(int i=0;i<numverticesprimal;i++){
		...
		ttt::SkeletonVertexType vertex = boost::add_vertex(newPoint,*descriptor->m_SkeletonGraph);

	 }

	 for(int i=0;i<numedgesprimal;i++){
		 ttt::SkeletonVertexType src = root["primal"]["edges"][i]["src"].asUInt64();
		 ttt::SkeletonVertexType tgt = root["primal"]["edges"][i]["tgt"].asUInt64();
	 ...
	 for(int i=0;i<numverticesdual;i++){
		...
		for(int j=0;j<perimeterLength;j++){
			newCell.AddVertexToPerimeter(root["dual"]["vertices"][i]["perimeter"][j].asUInt64());
		}

		ttt::CellVertexType vertex = boost::add_vertex(newCell,*descriptor->m_CellGraph);
 ...
 bool ttt::JSONTissueTrackingProject2::IsTissueDescriptorAvailable(unsigned int frame){
	 std::stringstream fileNameStream;
	    fileNameStream << m_ProjectDirectory << "/" << "tissue_T" << frame <<".json";
 ...
 typename  ttt::TrackedTissueDescriptor::Pointer ttt::JSONTissueTrackingProject2::GetTrackedTissueDescriptor(unsigned int frame){
	 Json::Reader reader;
	 	 ...
	 	 ttt::TrackedTissueDescriptor::Pointer descriptor = ttt::TrackedTissueDescriptor::New();
	 	 ...
	 	 for(int i=0;i<numverticesdual;i++){
	 		...
	 		ttt::TrackedCellVertexType vertex = boost::add_vertex(newCell,*descriptor->m_CellGraph);
 ...
 bool ttt::JSONTissueTrackingProject2::IsTrackedTissueDescriptorAvailable(unsigned int frame){
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "trackedtissue_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,itk::Point<double,3> > ttt::JSONTissueTrackingProject2::GetCentroids(unsigned int frame){
	 Json::Reader reader;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "centroids" << "_T" << frame <<".json";
 ...
 void ttt::JSONTissueTrackingProject2::SetCentroids(unsigned int frame,const FeatureMap<CellVertexType,itk::Point<double,3> >  & centroids){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "centroids" << "_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,double> ttt::JSONTissueTrackingProject2::GetXX(unsigned int frame){
	 Json::Reader reader;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "xx" << "_T" << frame <<".json";
 ...
 void ttt::JSONTissueTrackingProject2::SetXX(unsigned int frame, const FeatureMap<TrackedCellVertexType,double> & xx){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "xx" << "_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,double> ttt::JSONTissueTrackingProject2::GetXY(unsigned int frame){
	Json::Reader reader;
	Json::Value root;
	std::stringstream fileNameStream;
 ...
 void ttt::JSONTissueTrackingProject2::SetXY(unsigned int frame,const FeatureMap<TrackedCellVertexType,double> & xy){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "xy" << "_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,double> ttt::JSONTissueTrackingProject2::GetYY(unsigned int frame){
	Json::Reader reader;
	Json::Value root;
	std::stringstream fileNameStream;
 ...
 void ttt::JSONTissueTrackingProject2::SetYY(unsigned int frame, const FeatureMap<CellVertexType,double> & yy){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "yy" << "_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,double> ttt::JSONTissueTrackingProject2::GetPerimeter(unsigned int frame){
		Json::Reader reader;
		Json::Value root;
		std::stringstream fileNameStream;
 ...
 void ttt::JSONTissueTrackingProject2::SetPerimeter(unsigned int frame, const FeatureMap<CellVertexType,double> & perimeter){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "perimeter" << "_T" << frame <<".json";
 ...
 FeatureMap<CellVertexType,double> ttt::JSONTissueTrackingProject2::GetAreas(unsigned int frame){
	Json::Reader reader;
	Json::Value root;
	std::stringstream fileNameStream;
	fileNameStream << m_ProjectDirectory << "/" << "areas" << "_T" << frame << ".json";
 ...
 void ttt::JSONTissueTrackingProject2::SetAreas(unsigned int frame, const FeatureMap<CellVertexType,double> & areas){
	 Json::StyledWriter writer;
	 Json::Value root;
	 std::stringstream fileNameStream;
	 fileNameStream << m_ProjectDirectory << "/" << "areas" << "_T" << frame <<".json";
